        ...
        <div className=' flex  text-white w-full flex-col' >
            <div className='bg-white  h-10 w-full text-xl font-bold'>
                {['All', 'Active', 'Completed'].map((item) => {
                    return (
                        <Button key={item} className={`${menu == item ? 'bg-rose-700' : 'bg-black-800'} h-full w-1/3 trans border-none shadow-sm rounded-none shadow-black`} onPress={() => { item == menu ? setMenu('All') : setMenu(item) }}>
                            {item}
                        </Button>
                    )
                })}
            ...
            <div className='center-col text-white p-2 lg:px-40 border h-auto bg-white text-sm   md:text-base'>
                <div className='w-full fixed lg:px-40 top-10 shadow-sm font-bold bg-black text-whites  h-10  flex items-center justify-between px-4'>
                ...
                <div className="top-10 relative center-col w-full">
                    {(clientList).map((item) => {
                        return (
